...
public class jframe extends JFrame{
	...
	JTextField majors;
	JTextField Classes;
	String name1,major1,Classes1,result1;
	JLabel countdowns;
	JFrame frame;
	login login;
	...
	public jframe() {
		...
		began.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				name1 = names.getText();
				major1 = majors.getText();
				Classes1 = Classes.getText();
			}
		});
	...
	public  void GUIscreen() {
		...
		submit.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				// TODO Auto-generated method stub
				result1 = getresult(answerlist,rightkey);
				try {
					save_information(name1,major1,Classes1,result1);
	 ...
	 * 考试结束之后的结果显示界面和历史界面
	 */
	public void after_test(String results) {
		JFrame frame = new JFrame("广州市X小学数学考试自动系统");
		frame.getContentPane().setLayout(null);
		...
		JLabel label_6 = new JLabel(result1);
		label_6.setFont(new Font("楷体", Font.BOLD, 15));
		label_6.setBounds(157, 200, 108, 30);
		frame.getContentPane().add(label_6);
		frame.setVisible(true);
	}
 	public  void countdown(int endtime, JLabel countdowns1){
	...
	public String getresult(List<JTextField>answerlist,List<String>rightkey) {
		int b = 0;
		for(int a = 0;a<50;a++) {
			if(answerlist.get(a).getText().equals(rightkey.get(a))) {
				b+=2;
			}
			else {
				continue;
	...
	public void save_information(String name,String major,String Class,String result) throws IOException {
		try {
			BufferedWriter data = new BufferedWriter(new FileWriter("学生信息.txt",true));
			data.write(name+"\t");
			data.write(major+"\t");
			data.write(Class+"\t");
			data.write(result+"\t");
